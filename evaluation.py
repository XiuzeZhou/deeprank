...
def get_topk(prediction, test_sequence, topK=10):
    ...
    items_unrated = test_sequence[:,1:]
    prediction_not_rated = np.array([prediction[u,items_unrated[u,:]] for u in range(users_num)])
    y_pred_order = np.argsort(-prediction_not_rated)
